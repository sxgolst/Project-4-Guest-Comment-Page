...
class GuestHandler(webapp2.RequestHandler):
    ...
    def post(self):
        contact = Contact()
        contact.name = self.request.get('name').strip()
        contact.email = self.request.get('email').strip()
        contact.comment = self.request.get('content').strip()
...
app = webapp2.WSGIApplication([('/', GuestHandler),
